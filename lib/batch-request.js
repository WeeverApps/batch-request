...
var batchRequest = function(params) {
    ...
    var batch = function(req, res, next) {
            ...
            _.each(requests, function(requestValue, requestKey) {
                ...
                _.each(fileRefs, function(fileRef, fileRefsIndex) {
                    ...
                    _.each(fileRef, function(filesToSubstitute, fileKey) {
                        ...
                        // Temporarily storing file details until request().form() is available
                        requests[requestKey]['@files'] = [];

                        if (Array.isArray(filesToSubstitute)) {
                            filesToSubstitute.forEach(function(fileToSubstitute) {
                                var fileDetails = {
                                    filename: filesObject[fileToSubstitute].originalname,
                                    buffer: Buffer.from(filesObject[fileToSubstitute].buffer),
                                    key: fileKey
                                };
                                requests[requestKey]['@files'].push(fileDetails);
                            });
                        }
                        else {
                            var fileDetails = {
                                filename: filesObject[filesToSubstitute].originalname,
                                buffer: Buffer.from(filesObject[filesToSubstitute].buffer),
                                key: fileKey
                            };
                            requests[requestKey]['@files'].push(fileDetails);
                        }
        ...
        var requestPromise = function requestPromise(r) {
            ...
            var resultsOfRequest = request(r);

            var form = resultsOfRequest.form();

            atFiles.forEach(function(atFile) {
                form.append(atFile.key, atFile.buffer, {filename: atFile.filename});
            });
        ...
        var escapeSubstitutions = function escapeSubstitutions(/* literals, ...substitutions */) {
        ...
        var recurseDependencies = function recurseDependencies(reqs) {
                ...
                _.each(requestPromises, function(rp, key) {
                    ...
                    if (dependent) {
                        ...
                        requestPromises[dependentKey] = Promise.all(dependencyPromises).then(function(dependencyResponses) {
                            ...
                            // Stash this so it's not affected by the JSON round-trip
                            var atFiles = null;
                            if (dependent['@files']) {
                                atFiles = dependent['@files'];
                            ...
                            wx.console.log('atFiles')(atFiles);
                            parsedDependent['@files'] = atFiles;
