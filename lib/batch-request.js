    ...
    var batch = function(req, res, next) {
        ...
        if (files) {
            ...
            _.each(requests, function(requestValue, requestKey) {
                ...
                debug('fileRefs',fileRefs);
                _.each(fileRefs, function(fileRef, fileRefsIndex) {

                    _.each(fileRef, function(fileToSubstitute, fileKey) {
                        // fileToSubstitute will be a string that references the `name` of one of the originally uploaded files,
