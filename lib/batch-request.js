 ...
 *
 * @param {String} [params.formData.filesRef="filesRef"]
 *     Top-level property within each batch job that refers to a property within formData which should be parsed
 *     and have its string values replaced with the uploaded file stream with the corresponding name.
 *
 * @returns {batch}
 */
var batchRequest = function(params) {
    ...
    params.formData.filesRef = (typeof params.formData.filesRef === 'undefined') ? 'filesRef' : params.formData.filesRef;
    ...
    var batch = function(req, res, next) {
        ...
        if (files) {
            ...
            _.each(requests, function(requestValue, requestKey) {
                ...
                var filesRefValue = requestValue[params.formData.filesRef];
                debug( requestKey, requestValue, filesRefValue, requestValue.formData[filesRefValue] );

                _.each(requestValue.formData[filesRefValue], function(fileRefValue) {

                    // fileRefValue is the object with name and value (value might be an array in this case - ex. attachments:[]
                    _.each(fileRefValue, function(fileToSubstitute, fileKey) {
                        if (Array.isArray(fileToSubstitute)) {
                            // do something
                            // i should be able to use Array.map or something. basically:
                            // requests[requestKey].formData[fileKey] = [ substitutedFile[0], substitutedFile[1] ]
                        }
                        else {
                            // This should actually be stream(filesObject[fileToSubstitute].buffer)
                            requests[requestKey].formData[fileKey] = filesObject[fileToSubstitute];
                        }
                    });
                });
            });
